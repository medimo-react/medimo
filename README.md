...
|   <img src="https://github.com/yujsoo.png" width="80">    | 유지수 |      팀원      | - 카메라 촬영 및 갤러리 이미지 선택을 통한 OCR 스캔 라이브러리 연동<br/>- AI 분석 내용 목록 페이지 구현 (상단 고정 · 이름 변경 · 삭제 기능)<br/>               |   [GitHub](https://github.com/yujsoo)    |
| <img src="https://github.com/leopard0315.png" width="80"> | 김민혁 |      팀원      | - 로그인/회원가입 페이지 <br /> - 마이페이지 설정 및 계정 관리 <br />                                                   | [GitHub](https://github.com/leopard0315) |
...
| 이름     | 문제점                                                                                                                                                                                                                                                                                                                                            | 해결 방법                                                                                                                                                                                                                                                                                                                                                                                            |
| -------- |------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **유나** | 1. **약 이름 표시가 길어지는 문제**: 대시보드에서 약 이름과 괄호 안 성분명이 함께 표시되어 UI가 복잡해 보이는 문제 발생 </br> 2. **MongoDB Atlas 연결 실패 문제**: DB에 연결하려면 먼저 MongoDB 서버 주소를 찾아야 하는데, 일부 환경에서 그 주소를 제대로 못 찾는 문제가 있었음.                                                                                                                                                                                                                                                                                                                                         | 1. 괄호 안 내용을 제거하고 대표 약 이름만 보여주도록 문자열 가공 처리 </br> 2. Node 서버에서 사용할 DNS 서버를 직접 지정해 해결                                                                                                                                                                                                                                                                                                                                                                                               |
| **현주** | 배포시 http에러가 뜸                                                                                                                   | proxy 파일 추가함                                           |                                                                                                                                                                                                                                                                                                                                              | -                                                                                                                                                                                                                                                                                                                                                                                                |
| **지수** | 1. **Service Worker type: 'classic' → 'module' 오류** <br/> PWA 설정에서 vite-plugin-pwa가 dev 환경에서 service worker를 classic 타입으로 등록해서 모듈 import가 안 되는 에러 발생                                                                                                                                                                                                                                                                                     | 1. **Service Worker type: 'classic' → 'module' 오류** <br/> `devOptions: { type: 'module' }` 한 줄로 해결 — 에러 메시지와 원인이 직관적으로 연결 안 돼서 삽질하기 쉬운 케이스                                                                                                                                                                                                                                                                                                                                                                                              |
...
<details>
  <summary>지수</summary>
  <ul>
    <li>이번 프로젝트를 진행하면서 단순히 UI를 구현하는 것에서 나아가, 상태의 흐름과 데이터 구조를 함께 고려하는 과정의 중요성을 느꼈습니다. 특히 API 연동 과정에서 데이터가 어떻게 전달되고 관리되는지 이해하게 되면서, 서비스 전체를 바라보는 시야가 넓어졌습니다.</li>
  </ul>
</details>
<details>
  <summary>현주</summary>
